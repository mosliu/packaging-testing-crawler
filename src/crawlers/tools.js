...
function parseGotHref(base, href, crossSite = false) {
  ...
  if (crossSite === false) {
    ...
    // console.log('en.cscii.com'.indexOf('cscii.com'))
    if (refUrl.host.indexOf(baseUrl.host) > 0) {
      sameSiteFlag = sameSiteFlag || true;
    }
    if (baseUrl.host === refUrl.host) {
      sameSiteFlag = sameSiteFlag || true;
...
function judgedDocFile(href, websiteflag) {
  ...
  if (pathname.indexOf('.') === -1) {
    ...
    const suffixs = ['pdf', 'jpg', 'gif', 'png', 'mp4', 'wmv', 'mp3', 'wav', 'rm', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx'];
...
// console.log(parseGotHref(Url.parse('http://sumspring.cn'), 'http://www.themedica.com/'));
